...
#define ACCOUNT_QRY_ACCTINFO            6000    //账户信息查询
#define ACCOUNT_QRY_TRAN                6002    //行内交易结果查询

#define ACCOUNT_OUTCREDIT_DJ            6003    //提出贷录入冻结
#define ACCOUNT_OUTCREDITA              6004    //提出贷复核,针对6003
#define ACCOUNT_OUTCREDITB              6005    //提出贷复核,直接提出记账
#define ACCOUNT_OUTTRUNCDEBIT           6006    //提出借截留或即付入账
#define ACCOUNT_OUTNOTRUNCDEBIT         6007    //提出借非截留
#define ACCOUNT_INCREDIT                9006    //提入贷自动,同6006
#define ACCOUNT_INTRUNCDEBIT            9005    //提入借截留,同6005
#define ACCOUNT_INNOTRUNCDEBIT          6009    //提入借非截留挂账
#define ACCOUNT_INCREDIT_TP             9007    //提出贷被退票挂账,同6007
#define ACCOUNT_INCREDIT_TPCHK          9008    //提出贷被退票确认处理,同6008
#define ACCOUNT_OUTCREDIT_TP            9105    //提入贷作退票,同6005
#define ACCOUNT_INCREDIT_CHK            9108    //提入贷确认入账,同6008
#define ACCOUNT_INNOTRUNCDEBIT_CHK      6010    //提入借非截留确认
#define ACCOUNT_OUTDEBIT_TP             9010    //提入借非截留作退票,同6010
#define ACCOUNT_INDEBIT_TP              9208    //提出借被退票,同6008
#define ACCOUNT_OUTDEBIT_ST             9308    //提出借收妥,同6008
#define ACCOUNT_CREDITCZ                6011    //交易冲正
#define ACCOUNT_GET_MX                  6013    //取对账文件
...
int PF10_102(void *opDoc, char *p)
{
    ...
    iTranFlag = atoi(XMLGetNodeVal(doc, "//opReserved")); 
    ...
    if (p[0] == COMMTOPH_BEFORE[0])    //与中心通讯前
    {
        if (dcflag == OP_CREDITTRAN)
        {
            ...
            //网银业务直接记帐,柜面发起业务判断账户类型记帐
            if( iTranFlag == 99 )
                ret = Accounting(ACCOUNT_OUTCREDITB, doc);  //提出贷记
            else
            {
                iAcctFlag = GetAcctFlag( doc, ptmp+1 );
                if( iAcctFlag == 1 ) 
                    ret = Accounting(ACCOUNT_OUTCREDITA, doc);  //提出贷记(针对录入冻结)
                else if( iAcctFlag == 2 )
                    ret = Accounting(ACCOUNT_OUTCREDITB, doc);  //提出贷记(直接记帐)
            }
...
int PF10_104(void *opDoc, char *p)
{
    ...
    if (dcflag == OP_CREDITTRAN) //提入贷记直接返回成功
    {
        ...
        {
            if (ret = callInterface(ACCOUNT_QRY_ACCTINFO, doc)) {
                BKINFO("提入贷查询账户信息失败");
                return 0;
            }
        ...
        ret = Accounting(bktcode, doc); //贷记存款交易
    } 
    else if (dcflag == OP_DEBITTRAN)
    {
        // 地税发送过来的缴税交易
        ...
        ret = Accounting(bktcode, doc);  //提入借
    }
