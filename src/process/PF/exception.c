...
int SetException_10(xmlDoc *opDoc, char *rspbuf, int *rsplen, int operr)
{
    ...
    if (isInTran())
    {
        if (opBKRetcode != NULL && !sdpStringIsAllChar(opBKRetcode, '0'))
        {
            //查找行内错误码对相应前置的错误码
        ...
        if (operr) 
        {
            //查找平台错误码对相应前置的错误码,未找到或者找到为8999,则直接返回行内返回的错误
            ret = db_query_strs(vstrcat("SELECT mapcode, errinfo FROM errmap a, errinfo b WHERE "
            ...
            else
            {
                ...
                if (ret)
                {
                    ...
                    if( ret == E_DB_NORECORD )
                        strcpy(tmp, opTCRetinfo);
                    else
                        strcpy(tmp, "中心其它错误");
                } 
                /*返回接收机构内部错,则把返回的错误信息返回给前台*/
                if( atoi(opTCRetcode) == 8999 )
                {
                    sprintf(tmp + strlen(tmp), "%s%s",  "|", opTCRetinfo);
                }
            } 
        }
