...
/* 提出打印凭证查询 modify by luokf in yangzhou 2009/09/27
...
int QryOutNote( xmlDocPtr xmlReq, char *filename)
{
    ...
    int  exchground = -1;
    char tmp[128];
    char sAcctType[40+1], sSerial[8+1], sResult[8+1], sSqlStr[1024];
    char sMemo[64];
    char *p;
    int  suc_num = 0;
    ...
    iRet = db_query(&rs, 
            ...
            "benename, payer, trncode,truncflag FROM %s "
    ...
    for (i = 0; i < db_row_count(&rs); i++)
    {
        ...
        /* 对取到的日期和时间进行处理 add by luokf 20090927*/
        //分离出月、日，组成一个新串
        ...
        /* end by luokf in yangzhou 20090927*/
    ...
    //add by luokf 2009/10/21 in yangzhou 将金额从数字转换成字符串
    sprintf(charmoney, "%lf", suc_amt);
    //WriteRptFooter(fp, "%d;%.2lf;", suc_num, suc_amt);
    //add by luokf 2009/10/21 in yangzhou 将金额用带逗号的形式显示出来
...
//提入打印凭证查询,增加相应要素的中文显示 add by luokf 20090927
int QryInNote( xmlDocPtr xmlReq, char *filename)
{
    ...
    for (i = 0; i < db_row_count(&orglist); i++)
    {
        ...
        for (j = 0; j < db_row_count(&rs); j++)
        {
            ...
            //对票据种类进行处理
            //对票种类进行处理
            //到'noteinfo'表中去查询，对应票据的中文信息 add by luokf 20090927
            ...
            memset(sSqlStr, 0, sizeof sSqlStr);
            sprintf(sSqlStr, "select acctserial, result from acctjour where nodeid = %d and workdate = '%s' \
                    and originator = '%s' and convert(decimal, refid) = %ld and inoutflag = '%s'",
                    OP_REGIONID,
                    db_cell_by_name(&rs, 0, "workdate"),
                    db_cell(&rs, 0, 0),
                    atol(sSerial), //db_cell(&rs, 0, 1),
                    "2");
            memset(sResult, 0, sizeof sResult);
            memset(sSerial, 0, sizeof sSerial);
            iRet = db_query_strs(sSqlStr, sSerial, sResult);
