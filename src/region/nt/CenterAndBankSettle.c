...
#define org_name(orgid) \
    get_tb_value("select organname from organinfo where exchno='%s' and organlevel='1'", orgid)
...
int CenterAndBankSettle(char *dzRspFile, int classid, char *workdate, int workround)
{
    ...
    if ((p = org_name(bankid)) != NULL)
    {
        strncpy(bankname, p, sizeof(bankname)-1);
        free(p);
    }
    ...
    if (rc != 0)
    {
        if (GetError() != E_GNR_RECNOTFOUND)
            return -2;
        SetError(0);
        DBUG("E_GNR_RECNOTFOUND");
        return 0;
    }

    for (i = 0; i < rs1->row_count; i++)
    {
...
int CenterAndBankDiffer(FILE *fp, int classid, char *workdate, int workround)
{
    ...
    else
    {
        for (i = 0; i < rs->row_count; i++)
        {
            ...
            if (IsRecExist("bankjour", condi) == FALSE)
            {
