...
int CenterAndBankSettle(char *dzRspFile, int classid, char *workdate, int workround)
{
    ...
    snprintf(caParaFile, sizeof(caParaFile), "%s/dat/%d/SettleRpt.para", OP_HOME, TCOP_BANKID);
    ...
    for (i = 0; i < db_row_count(&rs1); i++)
    {
        ...
        // 提出借
        rc = db_query(&rs2, "select count(*), isnull(sum(settlamt),0.00) from bankjour "
        ...
        for (i = 0; i < db_row_count(&rs); i++)
        {
            ...
            if (db_hasrecord("bankjour", condi) == FALSE)
            {
                ...
                fprintf(fp, "结算中心;%s;%s;%s;%s;%s;%s;%s;%s;%s;\n",
                        ...
                        dctype, db_cell(&rs, i, 8), db_cell(&rs, i, 4), db_cell(&rs, i, 5),
                        db_cell(&rs,i, 6), db_cell(&rs, i, 7));
                count++;
            }
            else
            {
                // 结算中心与综合业务系统一致
                rc = db_exec("update bankjour set flag='1' where %s", condi);
                if (rc != 0)
                {
                    db_free_result(&rs);
                    return -1;
