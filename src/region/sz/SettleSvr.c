...
int RecTrnjourByNode(xmlNodePtr node)
{
    ...
    inoutflag = XmlGetInteger(doc, "//InOutFlag");

    if (inoutflag == 1)
        ret = db_query_str(innerorgid, sizeof(innerorgid),
                "select bankid from bankinfo where exchno='%s'", 
                XMLGetNodeVal(doc, "//Originator"));
    else
        ret = db_query_str(innerorgid, sizeof(innerorgid),
                "select bankid from bankinfo where exchno='%s'", 
                XMLGetNodeVal(doc, "//Acceptor"));
    if (ret != 0)
    {
        INFO("查找行内机构失败!");
        return ret;
    }
    XmlSetString(doc, "//opInnerorganid", innerorgid);

    if (InsertTableByID(doc, "trnjour", 901) != 0)
        return -2;
...
int RecBankJour(int svcclass, char *workdate, int workround)
{
    ...
    result = db_exec("delete from bankjour where nodeid=%d and workdate='%s' and classid=%d and workround LIKE '%s%%'", 
            OP_REGIONID, workdate, svcclass, workround == 0 ? "" : vstrcat("%d", workround));
